...
export class KafkaConsumerController {
  ...
  async handleReservationCreated(@Payload() message: any, @Ctx() context: KafkaContext) {
    try {
      this.logger.log(`📩 [EMAIL] Enviando confirmação de reserva para usuário ${message.user_id}...`);
      
    } catch (error) {
      // 2. Se der erro, mandamos para a DLQ
      await this.sendToDlq(KAFKA_TOPICS.DLQ.RESERVATION_CREATED, message, error);
    }
  }
  ...
  async handlePaymentConfirmed(@Payload() message: any) {
    try {
      this.logger.log(`[TICKET] Gerando ingresso para a venda ${message.sale_id}...`);
    } catch (error) {
  ...
  private async sendToDlq(topic: string, originalMessage: any, error: any) {
    this.logger.error(`Erro ao processar mensagem. Enviando para DLQ: ${topic}`, error.stack);
